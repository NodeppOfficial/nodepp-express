...
public: ssr() noexcept : obj( new NODE() ) { }
    ...
    coBegin;
        ...
        } while(0); coWait( write( &str, obj->borrow )==1 ); }
...
public: query_t params;
   ...
   ~express_http_t () noexcept { if( exp.count() > 1 ){ return; } exp->state=0; free(); }
    ...
    const express_http_t& render( string_t path ) const noexcept {
        ...
        auto self = type::bind( this );
        auto task = express::ssr(); send(); 
        process::add( task, self, path );
    ...
    const express_http_t& render( file_t file ) const noexcept {
        ...
        auto self = type::bind( this );
        auto task = express::ssr(); send(); 
        process::add( task, self, stream::await( file ) );
...
namespace nodepp { namespace express { namespace http {
    ...
    inline express_tcp_t file( string_t base ) { express_tcp_t app;
