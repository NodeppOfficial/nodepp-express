...
public:
    ...
    template< class T >
    coEmit( const T& file, const ptr_t<object_t>& done, const string_t& raw ) {
        thread_local static regex_t reg8 = regex_t( "Content-Disposition", false );
        thread_local static regex_t reg7 = regex_t( "\r\n\r\n", true );
    ...
    coBegin
        ...
        do { 
            ...
            auto hdr = raw.slice_view( 0, pos[0] );
            auto bdy = raw.slice_view(/**/pos[1] );
            auto sby = bdy.slice_view( 0, 1024   ); header_t obj;
...
public: query_t params;
   ...
   ~express_https_t () noexcept { if( exp.count() > 1 ){ return; } exp->state=0; free(); }
    ...
    promise_t<object_t,except_t> parse_stream() const noexcept {
        ...
        auto read = type::bind( generator::file::split() );
    ...
    return promise_t<object_t,except_t>( [=]( function_t<void,object_t> res, function_t<void,except_t> rej ){
        ...
        if( string::to_u64( self->headers["Content-Length"])==0 )
          { rej( except_t ( "content length mismatch" ) ); return; }

        thread_local static regex_t reg6 = regex_t( "boundary=[^ ]+", true );
        auto len = type::bind( string::to_u64( self->headers["Content-Length"] ) );
        auto bon = "--" + reg6.match( self->headers["Content-Type"] ).slice_view(9);

        if( bon.empty() ){ 
            res(json::parse(query::parse(url::normalize("?"+self->read())))); 
            return; 
        }
    ...
    const express_https_t& send( const string_t& msg ) const noexcept {
    ...
    const express_https_t& send_file( const string_t& dir ) const noexcept {
        if( exp->state==0 ){ return (*this); }
    ...
    template< class T >
    const express_https_t& send_stream( const T& readableStream ) const noexcept {
        if( exp->state == 0 ){ return (*this); }
    ...
    const express_https_t& send_json( const object_t& json ) const noexcept {
    ...
    const express_https_t& cookie( const string_t& name, const string_t& value ) const noexcept {
        if( exp->state == 0 ){ return (*this); } exp->_cookies[ name ] = value;
        header( "Set-Cookie", cookie::format( exp->_cookies ) );
        return (*this);
    }

    const express_https_t& header( const string_t& name, const string_t& value ) const noexcept {
        if( exp->state == 0 )    { return (*this); }
        exp->_headers[name]=value; return (*this);
    }

    const express_https_t& redirect( uint value, const string_t& url ) const noexcept {
        if( exp->state == 0 ){ return (*this); }
        header( "location",url ); status( value );
        send(); /*----------*/ return (*this);
    }

    const express_https_t& header( const header_t& headers ) const noexcept {
        if( exp->state == 0 ){ return (*this); }
    ...
    const express_https_t& redirect( const string_t& url ) const noexcept {
        if( exp->state == 0 ){ return (*this); }
...
protected:
    ...
    struct NODE {
        queue_t<express_item_t> list; tls_t fd;
    ...
    void execute( const string_t& path, express_item_t& data, express_https_t& cli, function_t<void>& next ) const noexcept {
        if  ( data.data.has_value() && data.mode==0 ){ data.data.as<MIDDL>()( cli, next ); return; }
        elif( data.data.has_value() && data.mode==1 ){ data.data.as<CALBK>() /*----*/ ( cli ); }
        elif( data.data.has_value() && data.mode==2 ){ data.data.as<ROUTR>().run( path, cli ); }
    next(); }

    /*.......................................................................*/

    void insert( const string_t& path, const express_item_t& item ) const noexcept {
    ...
    int path_match( express_https_t& cli, const string_t& base, const string_t& path ) const noexcept {
    ...
    void run( const string_t& path, express_https_t& cli ) const noexcept { do {
...
public:
    ...
    const express_tls_t& RAW( const string_t& _method, const string_t& _path, const CALBK& cb ) const noexcept {
    ...
    const express_tls_t& RAW( const string_t& _method, const CALBK& cb ) const noexcept {
        return RAW( _method, nullptr, cb );
    }

    const express_tls_t& RAW( const CALBK& cb ) const noexcept {
        return RAW( nullptr, nullptr, cb );
    }

    /*.........................................................................*/

    const express_tls_t& USE( const string_t& _path, const ROUTR& cb ) const noexcept {
        express_item_t item; // memset( (void*) &item, 0, sizeof(item) );
    ...
    const express_tls_t& USE( const ROUTR& cb ) const noexcept {
        return USE( nullptr, cb );
    }

    /*.........................................................................*/

    const express_tls_t& USE( const string_t& _path, const MIDDL& cb ) const noexcept {
        express_item_t item; // memset( (void*) &item, 0, sizeof(item) );
    ...
    const express_tls_t& USE( const MIDDL& cb ) const noexcept {
        return USE( nullptr, cb );
    }

    /*.........................................................................*/

    const express_tls_t& ALL( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( nullptr, _path, cb );
    }

    const express_tls_t& ALL( const CALBK& cb ) const noexcept {
        return RAW( cb );
    }

    /*.........................................................................*/

    const express_tls_t& GET( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "GET", _path, cb );
    }

    const express_tls_t& GET( const CALBK& cb ) const noexcept {
        return RAW( "GET", cb );
    }

    /*.........................................................................*/

    const express_tls_t& POST( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "POST", _path, cb );
    }

    const express_tls_t& POST( const CALBK& cb ) const noexcept {
        return RAW( "POST", cb );
    }

    /*.........................................................................*/

    const express_tls_t& QUERY( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "QUERY", _path, cb );
    }

    const express_tls_t& QUERY( const CALBK& cb ) const noexcept {
        return RAW( "QUERY", cb );
    }

    /*.........................................................................*/

    const express_tls_t& REMOVE( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "DELETE", _path, cb );
    }

    const express_tls_t& REMOVE( const CALBK& cb ) const noexcept {
        return RAW( "DELETE", cb );
    }

    /*.........................................................................*/

    const express_tls_t& PUT( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "PUT", _path, cb );
    }

    const express_tls_t& PUT( const CALBK& cb ) const noexcept {
        return RAW( "PUT", cb );
    }

    /*.........................................................................*/

    const express_tls_t& HEAD( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "HEAD", _path, cb );
    }

    const express_tls_t& HEAD( const CALBK& cb ) const noexcept {
        return RAW( "HEAD", cb );
    }

    /*.........................................................................*/

    const express_tls_t& TRACE( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "TRACE", _path, cb );
    }

    const express_tls_t& TRACE( const CALBK& cb ) const noexcept {
        return RAW( "TRACE", cb );
    }

    /*.........................................................................*/

    const express_tls_t& PATCH( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "PATCH", _path, cb );
    }

    const express_tls_t& PATCH( const CALBK& cb ) const noexcept {
        return RAW( "PATCH", cb );
    }

    /*.........................................................................*/

    const express_tls_t& OPTIONS( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "OPTION", _path, cb );
    }

    const express_tls_t& OPTIONS( const CALBK& cb ) const noexcept {
        return RAW( "OPTION", cb );
    }

    /*.........................................................................*/

    const express_tls_t& CONNECT( const string_t& _path, const CALBK& cb ) const noexcept {
        return RAW( "CONNECT", _path, cb );
    }

    const express_tls_t& CONNECT( const CALBK& cb ) const noexcept {
        return RAW( "CONNECT", cb );
    }

    /*.........................................................................*/

    template<class... T> expected_t<tls_t,except_t> 
    listen( const T&... args ) const noexcept {
        auto self = type::bind( this );
        auto ssl  = obj->ssl;
        ...
        obj->fd=https::server( cb, &ssl, &obj->agent );
        auto err = obj->fd.listen( args... ); 
        
        if( err.has_value() ){ return obj->fd; } else { return err; }
    }
...
namespace nodepp { namespace express { namespace https {
    ...
    template< class... T > 
    express_tls_t add( const T&... args ) { return ROUTR(args...); }

    inline express_tls_t file( const string_t& base ) {
    ...
    express_tls_t app; app.ALL([=]( express_https_t& cli ){
        ...
        if( !cli.headers.has("Range") ){
            ...
            array_t<string_t> range = reg[2].match_all(cli.headers["Range"]);
               ulong rang[3]; rang[0] = string::to_u64( range [0]  );
                     rang[1] =min(rang[0]+CHUNK_MB(10),str.size()-1);
                     rang[2] =min(rang[0]+CHUNK_MB(10),str.size()  );

            cli.header( "Content-Range", regex::format("bytes ${0}-${1}/${2}",rang[0],rang[1],str.size()) );
