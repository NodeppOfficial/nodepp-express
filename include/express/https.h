...
protected:
    ...
    int path_match( express_https_t& cli, string_t base, string_t path ) const noexcept {
    ...
    void run( string_t path, express_https_t& cli ) const noexcept { do {
...
public:
    ...
    const express_tls_t& USE( string_t _path, ROUTR cb ) const noexcept {
        express_item_t item; // memset( (void*) &item, 0, sizeof(item) );
    ...
    const express_tls_t& USE( ROUTR cb ) const noexcept {
...
namespace nodepp { namespace express { namespace https {
    ...
    inline express_tls_t file( string_t base ) {
